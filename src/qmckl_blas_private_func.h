...
#ifdef HAVE_DEVICE_POINTERS
#define qmckl_vec_device(v, i) v.data_device[i]
#define qmckl_mat_device(m, i, j) m.data_device[(i) + (j)*m.size[0]]

#define qmckl_ten3_device(t, i, j, k) t.data_device[(i) + t.size[0]*((j) + t.size[1]*(k))]
#define qmckl_ten4_device(t, i, j, k, l) t.data_device[(i) + t.size[0]*((j) + t.size[1]*((k) + t.size[2]*(l)))]
#define qmckl_ten5_device(t, i, j, k, l, m) t.data_device[(i) + t.size[0]*((j) + t.size[1]*((k) + t.size[2]*((l) + t.size[3]*(m))))]
#endif
