...
#if defined(HAVE_TREXIO) && defined(HAVE_DEVICE_POINTERS)
qmckl_exit_code
qmckl_trexio_read_nucleus_X_device(qmckl_context context, trexio_t* const file, int device_id)
{
  assert (context != (qmckl_context) 0);
  assert (file != NULL);
...

{
  ...
  if (shell_ang_mom == NULL) {
    return qmckl_failwith( context,
                           QMCKL_ALLOCATION_FAILED,
                           "qmckl_trexio_read_basis_shell_ang_mom_X_device",
                           NULL);
  }
...

{
  ...
  if (shell_prim_index == NULL) {
    return qmckl_failwith( context,
                           QMCKL_ALLOCATION_FAILED,
                           "qmckl_trexio_read_basis_shell_prim_index_X_device",
                           NULL);
  }
...

{
  ...
  if (exponent == NULL) {
    return qmckl_failwith( context,
                           QMCKL_ALLOCATION_FAILED,
                           "qmckl_trexio_read_basis_exponent_X_device",
                           NULL);
  }
...
#if defined(HAVE_TREXIO) && defined(HAVE_DEVICE_POINTERS)
qmckl_exit_code
qmckl_trexio_read_mo_X_device(qmckl_context context, trexio_t* const file, int device_id)
{
  assert (context != (qmckl_context) 0);
  assert (file != NULL);
...
qmckl_exit_code
qmckl_trexio_read_device(const qmckl_context context, const char* file_name, const int64_t size_max, int device_id)
{
  if (qmckl_context_check(context) == QMCKL_NULL_CONTEXT) {
    return false;
  }
