...
** Nucleus (device)
   ...
   In this section we read the number of nuclei, the molecular geometry and nuclear charges. Arrays are allocated on device.
...
** Basis set and AOs (device)

   In this section we read the atomic basis set and atomic orbitals. Arrays are allocated on device.
...
** Molecular orbitals (device)

   In this section we read the MO coefficients. Arrays are allocated on device.
...
{
  printf("Debut read device\n");
  if (qmckl_context_check(context) == QMCKL_NULL_CONTEXT) {
    printf("Pas de context");
    return false;
  }
  ...
  printf("Compil sans trexio\n");
  rc = qmckl_failwith( context,
                       QMCKL_FAILURE,
                       "qmckl_trexio_read",
                       "QMCkl was compiled without TREXIO");
#endif
  printf("Fin lect device\n");
  return rc;
}
#endif
  #+end_src
