...
qmckl_exit_code qmckl_provide_mo_vgl(qmckl_context context)
{
  ...
  if (ctx->point.date > ctx->mo_basis.mo_vgl_date) {
    ...
    rc = qmckl_compute_mo_basis_mo_vgl_offload_openACC(context,
                                       ctx->ao_basis.ao_num,
                                       ctx->mo_basis.mo_num,
                                       ctx->point.num,
    ...
    /*rc = qmckl_compute_mo_basis_mo_vgl_offload_openmp(context,
                                       ctx->ao_basis.ao_num,
                                       ctx->mo_basis.mo_num,
                                       ctx->point.num,
                                       ctx->mo_basis.coefficient_t,
...
#ifdef HAVE_OPENACC_OFFLOAD
qmckl_exit_code
qmckl_compute_mo_basis_mo_vgl_offload_openACC (const qmckl_context context,
                                   const int64_t ao_num,
                                   const int64_t mo_num,
                                   const int64_t point_num,
                                   const double* coef_normalized_t,
                                   ...
                                   double* restrict const mo_vgl )
{
  ...
  #pragma acc parallel copyout(coef_normalized_t[0:ao_num*mo_num],\
                                    ao_vgl[0:point_num*5*ao_num],\
                                    mo_vgl[0:point_num*5*mo_num])\
                                copyin(vgl1,\
                                    avgl1,\
  ...
  #pragma acc loop independent gan worker vector
  for (int64_t ipoint=0 ; ipoint < point_num ; ++ipoint) {
    vgl1 = &(mo_vgl[ipoint*5*mo_num]);
    avgl1 = &(ao_vgl[ipoint*5*ao_num]);
    vgl2 =  vgl1 + mo_num;
