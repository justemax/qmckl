                            ...
                            double* const mo_vgl )
{
  ...
  return qmckl_compute_mo_basis_mo_vgl_offload_openmp( context, ao_num, mo_num, point_num, coef_normalized_t, ao_vgl, mo_vgl);
//#elseif HAVE_HPC
...
#ifdef HAVE_OPENMP_OFFLOAD
qmckl_exit_code
qmckl_compute_mo_basis_mo_vgl_offload_openmp (const qmckl_context context,
                                   const int64_t ao_num,
                                   const int64_t mo_num,
                                   const int64_t point_num,
                                   const double* coef_normalized_t,
                                   ...
                                   double* restrict const mo_vgl )
{
    ...
    double* vgl1 ;// malloc(sizeof(mo_vgl));
    double* avgl1 ;// malloc(sizeof(ao_vgl));
    double* vgl2 ;// malloc(sizeof(vgl1) + (mo_num));
    double* vgl3 ;// malloc(sizeof(vgl1) + (mo_num));
    double* vgl4 ;// malloc(sizeof(vgl1) + (mo_num));
    double* vgl5 ;// malloc(sizeof(vgl1) + (mo_num));
    double* avgl2 ;// malloc(sizeof(avgl1) + (ao_vgl));
    double* avgl3 ;// malloc(sizeof(avgl1) + (ao_vgl));
    double* avgl4 ;// malloc(sizeof(avgl1) + (ao_vgl));
    double* avgl5 ;// malloc(sizeof(avgl1) + (ao_vgl));

    int64_t * idx = malloc(sizeof(int64_t)*ao_num);
    double * av1 = malloc(sizeof(double)*ao_num);
    double * av2 = malloc(sizeof(double)*ao_num);
    double * av3 = malloc(sizeof(double)*ao_num);
    double * av4 = malloc(sizeof(double)*ao_num);
    double * av5 = malloc(sizeof(double)*ao_num);

    double* ck1 ;// malloc(sizeof(coef_normalized_t)*ao_num*sizeof(idx));
    double* ck2 ;// malloc(sizeof(coef_normalized_t)*ao_num*sizeof(idx));
    double* ck3 ;// malloc(sizeof(coef_normalized_t)*ao_num*sizeof(idx));
    double* ck4 ;// malloc(sizeof(coef_normalized_t)*ao_num*sizeof(idx));

    double* ck ;// malloc(sizeof(coef_normalized_t) + sizeof(idx)*mo_num);



  #pragma omp target enter data map(to:coef_normalized_t[0:ao_num*mo_num],\
  ...
  for (int64_t ipoint=0 ; ipoint < point_num ; ++ipoint) {
    vgl1 = &(mo_vgl[ipoint*5*mo_num]);
    avgl1 = &(ao_vgl[ipoint*5*ao_num]);
    vgl2 =  vgl1 + mo_num;
    vgl3 =  vgl1 + (mo_num << 1);
    vgl4 =  vgl1 + (mo_num << 1) + mo_num;
    vgl5 =  vgl1 + (mo_num << 2);
    avgl2 = avgl1 + ao_num;
    avgl3 = avgl1 + (ao_num << 1);
    avgl4 = avgl1 + (ao_num << 1) + ao_num;
    avgl5 = avgl1 + (ao_num << 2);
    ...
    for (int64_t n=0 ; n < nidx-4 ; n+=4) {
      ck1 = coef_normalized_t + idx[n  ]*mo_num;
      ck2 = coef_normalized_t + idx[n+1]*mo_num;
      ck3 = coef_normalized_t + idx[n+2]*mo_num;
      ck4 = coef_normalized_t + idx[n+3]*mo_num;
    ...
    for (int64_t m=n0 ; m < nidx ; m+=1) {
      ck = coef_normalized_t + idx[m]*mo_num;
      double a1 = av1[m];
      double a2 = av2[m];
      double a3 = av3[m];
      double a4 = av4[m];
  ...
  #pragma omp target exit data map(from:mo_vgl[0:point_num*5*mo_num])
  return QMCKL_SUCCESS;
  }
#endif
    #+end_src
