...
qmckl_exit_code qmckl_set_electron_rescale_factor_en (qmckl_context context, const double kappa_en);
   ...
   #+begin_src c :comments org :tangle (eval h_func)
#ifdef HAVE_DEVICE_POINTERS
qmckl_exit_code qmckl_set_electron_coord_device    (qmckl_context context, const char transp, const double* coord, const int64_t size_max, int device_id);
#endif
   #+end_src
                                ...
                                int device_id)
{
  ...
  qmckl_exit_code rc;
  rc = qmckl_set_point_device(context, transp, coord, size_max/3, device_id);
  assert (rc == QMCKL_SUCCESS);
...
qmckl_exit_code qmckl_provide_ee_distance(qmckl_context context)
{
  ...
  if (ctx->electron.coord_new_date > ctx->electron.ee_distance_date) {
    ...
    //CALL C COMPUTE EE
    qmckl_exit_code rc =
      qmckl_compute_ee_distance_c(context,
                                ctx->electron.num,
...
qmckl_exit_code qmckl_compute_ee_distance_c (
  ...
  printf("walk num number = %d", walk_num);
  for(int k = 0; k < walk_num; k++){/*
    if(qmckl_distance_c(context, 'T', 'T', elec_num, elec_num,
                        coord, elec_num*walk_num,
                        coord, elec_num*walk_num,
                        ee_distance, elec_num) != QMCKL_SUCCESS){*/

    if(qmckl_distance_c(context, 'T', 'T', elec_num, elec_num,
                        &coord[0 * walk_num * elec_num + k * elec_num + 0], elec_num*walk_num,
                        &coord[0 * walk_num * elec_num + k * elec_num + 0], elec_num*walk_num,
                        &ee_distance[k * elec_num * elec_num + 0 * elec_num + 0], elec_num) != QMCKL_SUCCESS){
...
// (1,0,0) == (0,1,0)
printf("ee_distance[elec_num] = %lf\n", ee_distance[elec_num]);
assert(ee_distance[1] == ee_distance[elec_num]);

// value of (1,0,0)
printf("ee_distance[1] = %lf\n", ee_distance[1]);
...
qmckl_exit_code qmckl_compute_en_distance_c (
          const qmckl_context context,
          const int64_t elec_num,
  ...
  for(int i = 0; i < walk_num; i++){
    if(qmckl_distance_c(context, 'T', 'T', elec_num, nucl_num,
                        &elec_coord, elec_num * walk_num,
                        &nucl_coord, nucl_num,
                        en_distance, elec_num) != QMCKL_SUCCESS){
      return QMCKL_FAILURE;
    }
  }
  return QMCKL_SUCCESS;
