...
<<pre2>>
  ...
  {
  printf("type_nucl_vect adr %p, type nuc first = %d\n", type_nucl_vector, type_nucl_vector[2]);
  }


    printf("debut nucl_vecctor_device\n");
  int32_t mask = 1 << 2;
...
#pragma omp target is_device_ptr(type_nucl_vector)
  {
  printf("type_nucl_vect size %d, type nuc first = %ld\n", sizeof(type_nucl_vector), type_nucl_vector[2]);
  }
...
#pragma omp target is_device_ptr(type_nucl_vector, new_array)
  {
  printf("type_nucl_vect size %d, type nuc first = %ld\n", sizeof(type_nucl_vector), type_nucl_vector[2]);
  }
...
<<pre2>>
  ...
  omp_target_memcpy(new_array, aord_vector, mem_info.size,
                    0, 0,
...
<<pre2>>
  ...
  omp_target_memcpy(new_array, bord_vector, mem_info.size,
                    0, 0,
                    device_id, omp_get_initial_device()
                    );
...
<<pre2>>
  ...
  omp_target_memcpy(new_array, cord_vector, mem_info.size,
                    0, 0,
                    device_id, omp_get_initial_device()
                    );
...
qmckl_exit_code qmckl_provide_een_rescaled_e(qmckl_context context)
{
  ...
  printf("inside provide een rescaled e juste avant distance\n");
...
qmckl_exit_code qmckl_provide_een_rescaled_e_deriv_e(qmckl_context context)
{
  ...
  if (ctx->date > ctx->jastrow.een_rescaled_e_deriv_e_date) {
    ...
    //CHANGE ICI POUR DERIV

    rc = qmckl_compute_factor_een_rescaled_e_deriv_e_c(context,
                                                     ctx->electron.walk_num,
                                                     ctx->electron.num,
                                                     ctx->jastrow.cord_num,
                                                     ctx->electron.rescale_factor_kappa_ee,
...
integer function qmckl_compute_factor_een_rescaled_e_deriv_e_f( &
  ...
  do nw = 1, walk_num
    do j = 1, elec_num
      do i = 1, elec_num
        ...
        do ii = 1, 3
          ...
          !print *, elec_dist_deriv_e(ii, i, j)

        end do
        elec_dist_deriv_e(4, i, j) = 2.0d0 * rij_inv
        !print *, elec_dist_deriv_e(4, i, j)
      end do
    ...
    do l = 1, cord_num
      ...
      do j = 1, elec_num
        do i = 1, elec_num
          ...
          !print *, een_rescaled_e_deriv_e(i, 4, j, l, nw);
    ...
    #+begin_src c :comments org :tangle (eval h_private_func) :noweb yes :exports none
   qmckl_exit_code qmckl_compute_factor_een_rescaled_e_deriv_e_c (
         const qmckl_context context,
         const int64_t walk_num,
         const int64_t elec_num,
         const int64_t cord_num,
...
qmckl_exit_code qmckl_compute_factor_een_rescaled_e_deriv_e_c (
  ...
  double* elec_dist_deriv_e = malloc(sizeof(double*) * elec_num * elec_num * 4);
...
qmckl_exit_code qmckl_provide_tmp_c(qmckl_context context)
{
  ...
  if (ctx->date > ctx->jastrow.tmp_c_date) {
    ...
    if (gpu_offload) {
#ifdef HAVE_DEVICE_PTR
      rc = qmckl_compute_tmp_c_device_ptr_offload(context,
                                              ctx->jastrow.cord_num,
                                              ctx->electron.num,
                                              ctx->nucleus.num,
                                              ctx->electron.walk_num,
...
**** DevicePtr offload
     #+begin_src c :comments org :tangle (eval c) :noweb yes
#if defined(HAVE_OPENMP_OFFLOAD) && defined(HAVE_DEVICE_PTR)
qmckl_exit_code
qmckl_compute_tmp_c_device_ptr_offload (const qmckl_context context,
                                 const int64_t cord_num,
                                 const int64_t elec_num,
                                 const int64_t nucl_num,
                                 const int64_t walk_num,
...
#ifdef HAVE_DEVICE_POINTERS
qmckl_exit_code
qmckl_get_jastrow_factor_een_deriv_e_device(qmckl_context context,
                                     double* const factor_een_deriv_e,
                                     const int64_t size_max,
                                            int device_id);
#endif
                                            ...
                                            int device_id)
{
  ...
  printf("Go device ptr jastrow brrrrrr\n");
  ...
  omp_target_memcpy(factor_een_deriv_e, ctx->jastrow.factor_een_deriv_e, sze*sizeof(double),
                    0, 0,
                    device_id, omp_get_initial_device())
    ;
...
qmckl_exit_code qmckl_provide_factor_een_deriv_e_device(qmckl_context context, int device_id)
{
  ...
  qmckl_exit_code rc;
  printf("Debut qmckl provide factor een deriv e device\n");
  ...
  /* Check if en rescaled distance derivatives is provided */
  printf("Before lkpm provide\n");
  rc = qmckl_provide_lkpm_combined_index(context);
  printf("lkpm provide done\n");
  if(rc != QMCKL_SUCCESS) return rc;
  ...
  if (ctx->date > ctx->jastrow.factor_een_deriv_e_date) {
    ...
    printf("Call compute factor een deriv e c \n");
    rc = qmckl_compute_factor_een_deriv_e_c(context,
                                          ctx->electron.walk_num,
                                          ctx->electron.num,
                                          ctx->nucleus.num,
                                          ctx->jastrow.cord_num,
...
#   #+CALL: generate_c_header(table=qmckl_factor_een_deriv_e_args,rettyp=get_value("CRetType"),fname=get_value("Name"))
    ...
    #+begin_src c :comments org :tangle (eval h_private_func) :noweb yes :exports none
    qmckl_exit_code qmckl_compute_factor_een_deriv_e_c (
      const qmckl_context context,
      const int64_t walk_num,
      const int64_t elec_num,
      const int64_t nucl_num,
    ...
    qmckl_exit_code qmckl_compute_factor_een_deriv_e_c (
      ...
      for(int nw = 0; nw < walk_num; nw++){
        for(int n =0; n < dim_cord_vect; n++){
          l = lkpm_combined_index[0 * dim_cord_vect + n];
          k = lkpm_combined_index[1 * dim_cord_vect + n];
          p = lkpm_combined_index[2 * dim_cord_vect + n];
          m = lkpm_combined_index[3 * dim_cord_vect + n];


          for(int a = 1; a < nucl_num; a++){
            cn = cord_vect_full[n * dim_cord_vect + a];
            if (cn != 0){
             for(int ii = 0; ii < 4; ii++){
               for(int j = 0; j < elec_num; j++){
                 factor_een_deriv_e[nw * 4 * elec_num + ii * elec_num + j] = factor_een_deriv_e[nw * 4 * elec_num + ii * elec_num + j] \
                   + tmp_c[nw * cord_num * (cord_num + 1) * nucl_num * elec_num + k * (cord_num + 1) * nucl_num * elec_num + m * nucl_num * elec_num + a * elec_num + j]    * een_rescaled_n_deriv_e[nw * (cord_num  + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + ii * elec_num + j]    + dtmp_c[nw * cord_num * (cord_num +1) * nucl_num * 4 * elec_num + k * (cord_num + 1) * nucl_num * 4 * elec_num + (m) * nucl_num * 4 * elec_num + a * 4 * elec_num + ii * elec_num + j]    * een_rescaled_n[nw * (cord_num + 1) * nucl_num * elec_num + m * nucl_num * elec_num + a * elec_num + j]    + tmp_c[nw * cord_num * (cord_num + 1) * nucl_num * elec_num + k * (cord_num + 1) * nucl_num * elec_num + m * nucl_num * elec_num + a * elec_num + j] *    een_rescaled_n[nw * (cord_num  + 1) * nucl_num * 4 * elec_num + m * nucl_num * 4 * elec_num + a * 4 * elec_num + ii * elec_num + j] * cn;
               }
             }

             cn = cn + cn;
             for(int j=0; j < elec_num ; j++){
               factor_een_deriv_e[nw * 4 * elec_num + 4 * elec_num + j] = factor_een_deriv_e[nw * 4 * elec_num + 4 * elec_num + j] \
                   + dtmp_c[nw * cord_num * (cord_num +1) * nucl_num * 4 * elec_num + k * (cord_num + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + 0 * elec_num + j] *   een_rescaled_n_deriv_e[nw * (cord_num  + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + 0 * elec_num + j]\
                  + dtmp_c[nw * cord_num * (cord_num +1) * nucl_num * 4 * elec_num + k * (cord_num + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + 1 * elec_num + j] *   een_rescaled_n_deriv_e[nw * (cord_num  + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + 1 * elec_num + j]\
                  + dtmp_c[nw * cord_num * (cord_num +1) * nucl_num * 4 * elec_num + k * (cord_num + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + 2 * elec_num + j] *   een_rescaled_n_deriv_e[nw * (cord_num  + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + 2 * elec_num + j]\
                 + dtmp_c[nw * cord_num * (cord_num +1) * nucl_num * 4 * elec_num + k * (cord_num + 1) * nucl_num * 4 * elec_num + (m) * nucl_num * 4 * elec_num + a * 4 * elec_num + 0 * elec_num + j] *   een_rescaled_n_deriv_e[nw * (cord_num  + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + 0 * elec_num + j]\
                  + dtmp_c[nw * cord_num * (cord_num +1) * nucl_num * 4 * elec_num + k * (cord_num + 1) * nucl_num * 4 * elec_num + (m) * nucl_num * 4 * elec_num + a * 4 * elec_num + 1 * elec_num + j] *   een_rescaled_n_deriv_e[nw * (cord_num  + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + 1 * elec_num + j]\
                  + dtmp_c[nw * cord_num * (cord_num +1) * nucl_num * 4 * elec_num + k * (cord_num + 1) * nucl_num * 4 * elec_num + (m) * nucl_num * 4 * elec_num + a * 4 * elec_num + 2 * elec_num + j] *   een_rescaled_n_deriv_e[nw * (cord_num  + 1) * nucl_num * 4 * elec_num + (m+l) * nucl_num * 4 * elec_num + a * 4 * elec_num + 2 * elec_num + j] * cn;
