...
#+end_src
  ...
  #+begin_src c :comments link :tangle (eval c) :noweb yes
#include "qmckl.h"
#include "assert.h"
#include <stdio.h>
...
*** C header
    ...
    qmckl_exit_code qmckl_distance_c (
          const qmckl_context context,
          const char transa,
          const char transb,
          const int64_t m,
...
qmckl_exit_code qmckl_distance_c (
  ...
  if (transab < 0){
    return QMCKL_INVALID_ARG_1;
  }


// check for LDA
  if (iand(transab,1) == 0 && lda < 3) {
     return QMCKL_INVALID_ARG_7;
  }

  if (iand(transab,1) == 1 && lda < m) {
     return QMCKL_INVALID_ARG_7;
  }

  if (iand(transab,2) == 0 && lda < 3) {
     return QMCKL_INVALID_ARG_7;
  }

  if (iand(transab,2) == 2 && lda < m) {
     return QMCKL_INVALID_ARG_7;
  }

  // check for LDB
  if (iand(transab,1) == 0 && ldb < 3) {
     return QMCKL_INVALID_ARG_9;
  }

  if (iand(transab,1) == 1 && ldb < n) {
     return QMCKL_INVALID_ARG_9;
  }

  if (iand(transab,2) == 0 && ldb < 3) {
     return QMCKL_INVALID_ARG_9;
  }

  if (iand(transab,2) == 2 && ldb < n) {
     return QMCKL_INVALID_ARG_9;
  }
// check for LDC
  if (ldc < m) {
